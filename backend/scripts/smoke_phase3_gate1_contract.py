...
def main() -> None:
    ...
    approval_required = blocked_payload.get(
        "requires_approval",
        blocked_payload.get("approval_required", False),
    )
    assert approval_required is False, blocked_payload
    assert blocked_payload.get("tool_result") is None, blocked_payload
    assert blocked_payload.get("verification_result") is None, blocked_payload
    assert blocked_payload.get("presentation_status") is None, blocked_payload

    route = blocked_payload.get("route")
    if route == "office_action_blocked":
        assert blocked_payload.get("task_id") is None, blocked_payload
        ...
        task_id = str(blocked_payload.get("task_id") or "")
