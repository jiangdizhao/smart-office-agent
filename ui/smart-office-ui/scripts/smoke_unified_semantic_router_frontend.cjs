...
requireText(continuousVoice, 'const current = controller()', 'Stable continuous voice controller snapshot')
requireText(continuousVoice, "await current.submit(transcript, 'voice')", 'Continuous voice unified submit')
requireText(continuousVoice, "routeOwner: 'unified_semantic_router'", 'Continuous voice route ownership')
requireText(continuousVoice, 'duplicate-transcript-suppressed', 'Continuous voice duplicate suppression')
requireText(continuousVoice, 'DUPLICATE_TRANSCRIPT_WINDOW_MS', 'Bounded duplicate window')
forbidText(continuousVoice, 'executeDeterministicDesktopCommand', 'Continuous voice route')
