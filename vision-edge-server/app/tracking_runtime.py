...
def xywh_to_bbox(values: Iterable[float]) -> dict[str, float]:
    ...
    x = float(np.clip(cx - width / 2.0, 0.0, max(0.0, 1.0 - width)))
    y = float(np.clip(cy - height / 2.0, 0.0, max(0.0, 1.0 - height)))
...
def hungarian_assign(costs: np.ndarray, max_cost: float) -> list[tuple[int, int]]:
    """Return a rectangular minimum-cost assignment without requiring SciPy.

    Disallowed pairs use ``BLOCKED_COST``. The internal minima start at positive infinity rather
    than the same sentinel, which guarantees progress even when an entire row is blocked.
    Returned pairs are filtered by ``max_cost``.
    """
    ...
    matrix = np.where(np.isfinite(original), original, BLOCKED_COST)
    transposed = False
        ...
        while True:
            ...
            for column in range(1, columns + 1):
                ...
                if minimum[column] < delta:
                    ...
                    column1 = column
            if not np.isfinite(delta) or column1 == 0:
                break
    ...
    for column in range(1, columns + 1):
        if p[column] == 0:
            continue
        pair = (
            (column - 1, int(p[column]) - 1)
            if transposed
            else (int(p[column]) - 1, column - 1)
        )
...
class KalmanXYWH:
    ...
    def __init__(self, measurement: np.ndarray) -> None:
        ...
        self.covariance = (
            np.diag([0.01, 0.01, 0.02, 0.02, 0.08, 0.08, 0.08, 0.08]) ** 2
        )
        self.measurement_matrix = np.eye(4, 8, dtype=np.float64)
    ...
    def update(self, measurement: np.ndarray) -> None:
        ...
        noise = (
            np.diag([0.03 * scale, 0.03 * scale, 0.06 * scale, 0.06 * scale]) ** 2
        )
        projected_mean = self.measurement_matrix @ self.mean
        projected_covariance = (
            self.measurement_matrix @ self.covariance @ self.measurement_matrix.T + noise
        )
        gain = (
            self.covariance
    ...
    def gating_distance(self, measurement: np.ndarray) -> float:
        ...
        return float(
            residual.T
            @ np.linalg.inv(projected_covariance + np.eye(4, dtype=np.float64) * 1e-5)
            @ residual
        )

    def observation_correction(
        self,
        previous_observation: np.ndarray,
        current_observation: np.ndarray,
        elapsed_seconds: float,
    ) -> None:
        """Correct velocity after a lost interval using the two real observations.

        This is a lightweight observation-centric recovery step: it does not reproduce the full
        OC-SORT virtual-trajectory algorithm, but prevents stale pre-occlusion velocity from
        dominating immediately after reactivation.
        """

        elapsed_seconds = max(float(elapsed_seconds), 1.0 / 60.0)
        observed_velocity = (current_observation - previous_observation) / elapsed_seconds
        observed_velocity[:2] = np.clip(observed_velocity[:2], -1.5, 1.5)
        observed_velocity[2:] = np.clip(observed_velocity[2:], -1.5, 1.5)
        self.mean[4:] = 0.75 * observed_velocity + 0.25 * self.mean[4:]
        self.covariance[4:, 4:] += np.eye(4, dtype=np.float64) * 0.01
...
class Track:
    ...
    def create(
        ...
        observation = bbox_to_xywh(detection["bbox"])
        track = cls(
            track_id=track_id,
            kalman=KalmanXYWH(observation),
    ...
    def update(
        ...
        measurement = bbox_to_xywh(detection["bbox"])
        previous_observation = self.last_observation.copy()
        self.kalman.update(measurement)
        if was_lost:
            self.kalman.observation_correction(
                previous_observation,
                measurement,
                lost_seconds,
            )
        self.last_observation = measurement.copy()
        ...
        if feature is not None:
            if self.feature is None or self.feature.size != feature.size:
                ...
                self.feature = (
                    (combined / norm).astype(np.float32)
                    if norm > 1e-12
                    else combined.astype(np.float32)
                )
    ...
    def approaching_score(self) -> float:
        ...
        return float(np.clip(0.5 + 4.0 * rate, 0.0, 1.0))
    ...
    def snapshot(self, now: float, primary: bool) -> dict[str, Any]:
        ...
        return {
            ...
            "lost_seconds": (
                round(max(now - self.last_seen_at, 0.0), 3)
                if self.state == "lost"
                else 0.0
            ),
...
class PrimarySelector:
    ...
    def update(
        ...
        if (
            ...
            and now - current.last_seen_at <= self.settings.lost_lock_seconds
        ):
        ...
        if current is None:
            ...
            elif now - float(self.candidate_since if self.candidate_since is not None else now) >= self.settings.acquire_stable_seconds:
                self.primary_track_id = best.track_id
                self.candidate_id = None
                self.candidate_since = None
        elif best is not None and best.track_id != current.track_id:
            if (
                self.scores[best.track_id] - self.scores[current.track_id]
                >= self.settings.challenger_margin
            ):
                if self.challenger_id != best.track_id:
                    self.challenger_id = best.track_id
                    self.challenger_since = now
                elif now - float(
                    self.challenger_since if self.challenger_since is not None else now
                ) >= self.settings.challenger_hold_seconds:
                    self.primary_track_id = best.track_id
        ...
        return self.primary_track_id, {
            ...
            "reason": (
                "acquired" if previous is None else "previous_lost_or_challenged"
            ),
    ...
    def score(self, track: Track, now: float) -> float:
        ...
        zone = (
            1.0
            if point_in_polygon((x, y), self.presence.engagement_zone)
            else 0.0
        )
        area = float(
            np.clip(track.area_ratio / self.settings.area_full_scale_ratio, 0.0, 1.0)
        )
        centrality = float(
            np.clip(
                1.0
                - math.hypot(
                    x - self.settings.target_x,
                    y - self.settings.target_y,
                )
                / math.sqrt(2.0),
                0.0,
                1.0,
        ...
        dwell = float(
            np.clip(
                (now - track.created_at) / self.settings.dwell_full_scale_seconds,
                0.0,
                1.0,
            )
        )
        stability = float(
            np.clip(
                track.hits / self.settings.stability_full_scale_hits,
                0.0,
                1.0,
            )
        )
...
class MultiObjectTracker:
    ...
    def reset(self) -> None:
        with self._lock:
            ...
            self.primary.candidate_id = None
            self.primary.challenger_id = None
            self.update_count = 0
    ...
    def _update_locked(
        ...
        high = [
            ...
            if detection["score"] >= self.settings.high_threshold
        ]
        low = [
            index
            for index, detection in enumerate(detections)
            if self.settings.low_threshold
            <= detection["score"]
            < self.settings.high_threshold
        ]
        features = {
            index: self.appearance.extract(source_frame, detections[index]["bbox"])
            for index in high
        }
        ...
        stage1_tracks = [
            track for track in active if track.state in {"confirmed", "lost"}
        ]
        stage1_pairs = self.associate(
            stage1_tracks,
            high,
            detections,
            features,
        ...
        for track_row, detection_column in stage1_pairs:
            ...
            recovered, lost_seconds = track.update(
                detections[detection_index],
                now,
                features.get(detection_index),
                "high",
                self.settings.feature_ema_alpha,
            )
        ...
        stage2_tracks = [
            track
            for track in active
            if track.state == "confirmed" and track.track_id not in matched_tracks
        ]
        remaining_low = [
            index for index in low if index not in matched_detections
        ]
        stage2_pairs = self.associate(
            stage2_tracks,
            remaining_low,
            detections,
            {},
        ...
        tentative_tracks = [
            track
            for track in active
            if track.state == "tentative" and track.track_id not in matched_tracks
        ]
        remaining_high = [
            index for index in high if index not in matched_detections
        ]
        tentative_pairs = self.associate(
            tentative_tracks,
            remaining_high,
            detections,
            features,
        ...
        for track_row, detection_column in tentative_pairs:
            ...
            matched_detections.add(detection_index)
            if (
                track.hits >= self.settings.confirm_hits
                and track.age_frames <= self.settings.confirm_window_frames
            ):
        ...
        for track in active:
            ...
            if (
                ...
                track.state == "lost"
                and now - track.last_seen_at > self.settings.lost_timeout_seconds
            ):
                track.state = "removed"
                track.removed_at = now
        ...
        for detection_index in high:
            if (
                ...
                or detections[detection_index]["score"]
                < self.settings.new_track_threshold
            ):
                continue
            track = Track.create(
                next(self.next_id),
        ...
        self.tracks = [
            ...
            or now - float(track.removed_at if track.removed_at is not None else now)
            <= self.settings.removed_retention_seconds
        ]
        ...
        for track in confirmed:
            in_zone = point_in_polygon(
                track.bottom_center,
                self.presence.engagement_zone,
            )
            if (
                track.visible
                and in_zone
                and track.area_ratio >= self.presence.engagement_min_area_ratio
            ):
                track.engagement_candidate_frames += 1
                if (
                    not track.engaged
                    and track.engagement_candidate_frames
                    >= self.presence.engage_confirm_frames
                ):
                    track.engaged = True
        ...
        snapshots = [
            track.snapshot(now, track.track_id == primary_id)
            for track in self.tracks
            if track.state != "removed"
        ]
    ...
    def associate(
        ...
        if not tracks or not detection_indices:
            return []
        costs = np.full(
            (len(tracks), len(detection_indices)),
            BLOCKED_COST,
            dtype=np.float64,
        )
        for row, track in enumerate(tracks):
            ...
            for column, detection_index in enumerate(detection_indices):
                ...
                center_distance = float(
                    np.linalg.norm(predicted_xywh[:2] - observed_xywh[:2])
                )
                allowed_center = (
                ...
                if (
                    center_distance > allowed_center
                    and iou < self.settings.minimum_iou_gate
                ):
                    continue
                if (
                    track.kalman.gating_distance(observed_xywh)
                    > self.settings.mahalanobis_gate
                    and iou < 0.05
                ):
                    continue
                ...
                if use_appearance:
                    ...
                    if similarity is not None:
                        if (
                            similarity < self.settings.appearance_similarity_gate
                            and iou < 0.10
                        ):
                            continue
                ...
                cost = (
                    ...
                    * min(center_distance / allowed_center, 1.0)
                )
                if use_appearance:
                    cost += self.settings.appearance_weight * appearance_distance
                else:
                    cost /= max(
                        self.settings.iou_weight + self.settings.motion_weight,
                        1e-9,
                    )
    ...
    def event_payload(self, track: Track, now: float) -> dict[str, Any]:
        payload = track.snapshot(
            now,
            track.track_id == self.primary.primary_track_id,
        )
        payload["visit_session_id"] = (
            f"visit_{track.track_id}_{int(track.created_at * 1000)}"
        )
        return payload
    ...
    def snapshot(self, now: float | None = None) -> dict[str, Any]:
        with self._lock:
            ...
            visible = [
                track
                for track in tracks
                if track["visible"] and track["state"] == "confirmed"
            ]
            primary = next((track for track in tracks if track["primary"]), None)
            scene_state = (
                "engaged"
                if primary and primary["engaged"]
                else "present"
                if visible
                else "absent"
            )
