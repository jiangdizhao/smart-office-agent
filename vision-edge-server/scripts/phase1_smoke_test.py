...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify the live camera and detector pipeline.")
    ...
    with httpx.Client(timeout=10.0) as client:
        while time.monotonic() < deadline:
            ...
            print("FAIL: Camera and detector pipeline did not become ready before timeout.", file=sys.stderr)
            print(last_status, file=sys.stderr)
            return 1

        detections = client.get(f"{args.base_url}/api/v1/detections")
        detections.raise_for_status()
        detections_payload = detections.json()
    ...
    person_count = detections_payload.get(
        "raw_person_count",
        detections_payload.get("person_count"),
    )
    ...
    print(f"Latest raw person count: {person_count}")
